...
*/
  ...
  const handleSubmit = async (event) => {
    ...
    if (monthIndex < 0 || !year || !day) {
      console.error("Invalid birth date input", form);
      return;
    }
    ...
    if (isNaN(date.getTime())) {
      console.error("Invalid constructed date", form);
      return;
    }

    user.DateOfBirth = date.toISOString().split("T")[0];
            ...
            <div className="flex gap-4 text-sm text-zinc-300">
              ...
              <label className="flex items-center gap-2">
                <input type="radio" name="gender" className="accent-rose"
                  ...
                  onChange={(e) => { setOtherSelected(e.target.checked); setForm((prev) => ({ ...prev, gender: "" })) }} />
                {'Other'}
              </label>
            </div>
            {otherSelected && (
